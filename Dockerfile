...
RUN echo "0 0 * * * /usr/local/bin/python /code/manage.py generate_tree > /var/log/generate_tree.log 2>&1 " >> /code/cronjob
RUN echo "0 * * * Mon /usr/local/bin/python /code/manage.py reset_container_limits > /var/log/reset_container_limits.log 2>&1 " >> /code/cronjob
